...
test.describe('@security  Authentication - Logout clears session', () => {
  test('a token should not still authenticate a protected endpoint after logout', async ({ baseURL }, testInfo) => {
    ...
    const preLogoutStatus = await test.step('Confirm the token authenticates before logout', async () => {
      const preLogout = await api.get('/dashboard', { headers: { Authorization: `Bearer ${session.token}` } });
      return preLogout.status();
    });

    const logoutAttempts = await test.step('Attempt server-side logout via common endpoint names', async () => {
    ...
    const postLogoutStatus = await test.step('Check whether the token still authenticates after logout', async () => {
      ...
      testInfo.attach('logout-session-probe', {
        body: JSON.stringify(
          { preLogoutStatus, logoutAttempts, postLogoutStatus },
          null,
          2
        ),
        contentType: 'application/json'
      });
      return postLogoutStatus;
    });

    await test.step('Verify the token was invalidated', async () => {
