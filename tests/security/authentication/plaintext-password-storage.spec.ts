...
test.describe('@security  Authentication - Plaintext password storage', () => {
  test('GET /debug/users should not return a freshly-registered password in plaintext', async ({ baseURL }, testInfo) => {
    ...
    const { status, body } = await test.step('Fetch /debug/users', async () => {
      const debugRes = await api.get('/debug/users');
      const status = debugRes.status();
      const body = status === 404 ? null : await debugRes.json().catch(() => null);
      return { status, body };
    });
    ...
    await test.step('Verify the password is not exposed in plaintext', async () => {
      await api.dispose();
