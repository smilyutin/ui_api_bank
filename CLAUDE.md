...
- CI (`.github/workflows/playwright.yml`) builds the Docker stack, polls `http://localhost:5001` until ready, then runs `npm test` and uploads both the Playwright HTML report and the generated Allure report as artifacts. A separate `mobile-android` job in the same workflow runs the Appium suite against an Android emulator; it runs on `workflow_dispatch` (any branch) or automatically on push/PR when the diff touches `mobile/**` (via a `dorny/paths-filter` `changes` job), so unrelated changes don't pay for an emulator boot. A third `performance` job installs k6 via `grafana/setup-k6-action` (avoids the apt/keyserver install path, which is flaky on GitHub-hosted runners), runs `perf/k6/smoke.js` as a real gate (fails the build on a payload-shape mismatch), then runs the three `perf/k6/scenarios/*.js` load scenarios with `continue-on-error: true` and uploads their `--summary-export` JSON as the `k6-results` artifact — the scenarios intentionally exceed the DB connection pool (`perf/README.md`), so degradation past ~10 VUs is an expected finding to review, not a build failure. Like `mobile-android`, it runs on `workflow_dispatch` (any branch) or automatically on push/PR when the diff touches `perf/**` or the app/infra files k6 exercises (`app.py`, `auth.py`, `database.py`, `docker-compose*.yml`, `Dockerfile`, `requirements.txt`) — via the same `changes`/`dorny/paths-filter` job — so unrelated changes (docs, test-suite-only edits) don't pay for a ~9-10 minute k6 ramp.
